...
구성/흐름:
1) 전역 상태/설정 (CURRENT_MAP_ID, ACTIVE_TILE_FOLDER, 캐시 등)
2) 경로 유틸 (_override_path, _blocks_path, _norm)
3) 맵 전환 API (set_current_map, get_current_map)
4) 타일 오버라이드 API (load_overrides, save_overrides, TILE_OVERRIDE)
5) 블록(벽) API (load_blocks, save_blocks, toggle_block_at_world, BLOCKS)
6) 좌표/그리드 유틸 (get_cell_from_world, world_to_block, clamp_to_world)
7) 렌더링 (draw_background, draw_blocks_overlay, load_image_cached)
8) 충돌 (circle-rect 교차, collides_circle)

의존:
- settings.py: SCREEN_W, SCREEN_H, TILE_SIZE, MAP_ROWS, MAP_COLS, WORLD_W, WORLD_H,
               TILE_FOLDER, BG_CLEAR_COLOR, BLOCK_SIZE
"""
...
# ---- settings에서 필요한 상수 임포트 ----
from settings import (
    SCREEN_W, SCREEN_H,
    TILE_SIZE, MAP_ROWS, MAP_COLS,
    WORLD_W, WORLD_H,
    TILE_FOLDER,          # 기본 타일 폴더
    BG_CLEAR_COLOR,       # 배경 클리어 색
    BLOCK_SIZE,           # 벽(충돌) 격자 크기(px)
)
...
# 현재 사용 중인 타일 폴더(맵 전환 시 변경 가능)
ACTIVE_TILE_FOLDER: str = TILE_FOLDER

# 타일 오버라이드: {(r,c): "assets/tiles/x.png"}
TILE_OVERRIDE: Dict[Tuple[int, int], str] = {}

# 이미지 캐시: {path: pygame.Surface or None}
...
def _norm(path: str) -> str:
    """슬래시 통일(윈도우 역슬래시 → /)하여 저장 안정성 확보."""
    return path.replace("\\", "/")

def _override_path(map_id: str) -> str:
    """오버라이드 JSON 파일 경로: map_overrides_{map}.json"""
    return f"map_overrides_{map_id}.json"

def _blocks_path(map_id: str) -> str:
    """벽(블록) JSON 파일 경로: map_blocks_{map}.json"""
...
def set_current_map(map_id: str, *, tile_folder: Optional[str] = None, autoload: bool = True) -> None:
    """
    현재 맵을 설정하고 필요 시 오버라이드/블록 데이터를 자동 로드.
    - map_id: "city", "bank" 등
    - tile_folder: 맵별 타일 폴더 (지정 없으면 기존 유지)
    - autoload: True면 load_overrides/load_blocks 실행
    """
    global CURRENT_MAP_ID, ACTIVE_TILE_FOLDER, TILE_OVERRIDE, BLOCKS, _image_cache
    CURRENT_MAP_ID = map_id
    if tile_folder is not None:
        ACTIVE_TILE_FOLDER = tile_folder
    # 맵 전환 시 캐시 비우기(타일/이미지)
    TILE_OVERRIDE.clear()
    _image_cache.clear()
    # 블록은 새 맵으로 바뀌니 비워두고 필요 시 로드
    BLOCKS.clear()
...
def load_overrides(map_id: Optional[str] = None) -> None:
    """
    오버라이드 JSON 로드. 새로 로드하면 이미지 캐시 초기화.
    JSON 포맷:
    - 최신: {"_meta": {...}, "overrides": {"r,c": "assets/tiles/x.png", ...}}
    - 구버전: {"r,c": "assets/tiles/x.png", ...} (호환 처리)
    """
    global TILE_OVERRIDE, _image_cache
    TILE_OVERRIDE.clear()
    _image_cache.clear()
...
def save_overrides(map_id: Optional[str] = None) -> None:
    """
    현재 오버라이드 상태를 JSON으로 저장.
    저장 시 경로는 슬래시(/)로 통일하여 OS 독립성 확보.
    """
    mid = map_id or CURRENT_MAP_ID
